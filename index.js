...
function createHeroCard(hero) {
    ...
    for (let i = 5; i >= 1; i--) {
        ...
        radio.type = 'radio';
        radio.id = `${hero.name}-${i}`;
        radio.name = hero.name;
        radio.value = i;
        radio.addEventListener('change', () => saveRating(hero.name, i));

        const label = document.createElement('label');
        label.htmlFor = `${hero.name}-${i}`;
    ...
    if (savedRating) {
        const radio = card.querySelector(`input[value="${savedRating}"]`);
        if (radio) {
            radio.checked = true;
        }
    }
...
function saveRating(heroName, rating) {
...
window.addEventListener('load', createHeroCards);